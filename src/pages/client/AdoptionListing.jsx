...
export default function AdoptionListing() {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
        ...
        if (s === "ADOPTED") {
          setBanner({ type: "success", text: "🎉 Chúc mừng! Yêu cầu nhận nuôi đã được CHẤP NHẬN." });
          clearInterval(timer);
          setWatchingId(null);
        ...
        if (s === "AVAILABLE") {
          setBanner({ type: "warning", text: "❌ Rất tiếc! Yêu cầu nhận nuôi đã bị TỪ CHỐI." });
          clearInterval(timer);
          setWatchingId(null);
          localStorage.removeItem("adoption_watching_pet");
          return;
...
await requestAdoption(active.id, {
      ...
      setBanner({ type: "info", text: "Đã gửi yêu cầu. Đang chờ shelter xét duyệt..." });
