      ...
      <div className="card h-100 shadow-sm" style={{ borderRadius: 16 }}>
        ...
        <div className="card-body d-flex flex-column">
          ...
          <div className="text-muted small mb-1">
            ...
            {vet.specialization || vet.speciality || "General"}
          </div>
  ...
  const handleSubmit = async (e) => {
    ...
    if (!petId || !date || !time) {
      await Swal.fire({
        ...
        text: "Vui lòng chọn thú cưng, ngày và giờ.",
        icon: "warning",
      ...
      await Swal.fire({
        ...
        html: `
          <div style="text-align:left">
            <div><b>Bác sĩ:</b> ${vet.specialization || "Veterinarian"}</div>
            <div><b>Thú cưng:</b> ${petName}</div>
            <div><b>Thời gian:</b> ${date} ${time}</div>
            <div><b>Lý do:</b> ${reason || "Vet appointment"}</div>
          </div>
        `,
        icon: "success",
  ...
  return (
    <div className="vf-backdrop">
      <div className="vf-modal p-4 bg-white rounded shadow">
        <h4 className="mb-3">
          Book Appointment with {vet.specialization || "Veterinarian"}
        </h4>
            ...
            <button
              ...
              onClick={async () => {
                const res = await Swal.fire({
                  title: "Huỷ đặt lịch?",
                  text: "Thông tin đã nhập sẽ không được lưu.",
                  icon: "question",
                  showCancelButton: true,
                  confirmButtonText: "Huỷ",
                  cancelButtonText: "Tiếp tục chỉnh",
  ...
  const handleCreateDefaultAddress = async () => {
    try {
      ...
      if (result.success) {
        ...
        await Swal.fire({
          title: "Đã tạo địa chỉ mặc định",
          html: `<div style="text-align:left">
                   <div><b>Thông báo:</b> ${result.message}</div>
                   <div><b>Địa chỉ:</b> ${result.address || "N/A"}</div>
                 </div>`,
          icon: "success",
          confirmButtonText: "OK",
        });
      ...
      await Swal.fire({
        title: "Lỗi",
        text: error.message || "Không thể tạo địa chỉ mặc định.",
        icon: "error",
      });
    } finally {
      setLoading(false);
    ...
    <section className="product-area section-space">
      <div className="container">
        <div className="section-title text-center">
          ...
          <p>
            Search for veterinarians by name, location, or specialization and
            book appointments.
          </p>
        </div>
        ...
        {!loading && list?.length > 0 && (
          <div className="row">
            {list.map((vet) => (
              <VetCard
                key={vet.id ?? vet.vetId ?? vet.vetUserId ?? vet.userId}
                vet={vet}
